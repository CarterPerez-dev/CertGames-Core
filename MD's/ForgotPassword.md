scan all my files for context and then add a forgot password fucntiolity to my forgot passwod file. so prbbaly gonna need a route file and maybe a helper/model file and then whats teh easiest forgot password sender and then kind aod the saem thing i do with my userproifle page where they can change tha passowrd so kind like that but thtis way it would send them a link to a page where i have teh chnage password fucntility and tehy woudlnt need to tupe in theri old password (obviolsy lol) it woul djust be new password and tp eit it again to verify. so yea that would be teh pmrpto and hey woudl need ot uputput any bakcend files needed paired with teh email sender ill use (prrferablly a free one) and then the frotnend page it sends them too and like after the make it it redirects them to login and stuff. 



so heres context-

this is like for login and regsiter and stuff liek that

here is alot of backend context files (but woudld just wnat to make  serpate hlper/model file and sperate rouet file but these files helpw tih conext becasue all of it should sorta be compabitle with each other in terms of pasword and stuff)|
...
ok now here is the forgot password page whcih i woudl assume needs a tiny bit of twekaing (DO NOT CHANEG THE DISGN AT ALL)
...
and yea so thats all the files that help with conetst aswell the forgotpassword js file

so i think all wee need ot do is decide a free amil sedner for forgot emails (prefferably easiest to integsrate aswell as able to mak the email domain my website domain custom) and choose for me, then make teh route and a model file if needed for it, then integrate it into the forgotpassword file and then make a file/js adn css (page) for the url that the forgot password link. so teh user gets the mail- clik teh link and it brigs them to the page where tehy chnage their password then after it redirect sthem to lgoin. cool??

ok andmake sure ot output all files in full
