// src/components/pages/testpage/APlusCore2TestPage.js
import React, {
  useState,
  useEffect,
  useCallback,
...
// CHANGED: Import the separate list component for Core 2, e.g. "APlusCore2TestList"
import APlusCore2TestList from "./AplusCore2TestList"; 

import "./APlusCore2TestPage.css";  // Reuse your APlus styles if desired
...
/* ------------------------------------------------------------------
   Main Component: APlusCore2TestPage
   Renders the test list if no :testId is provided,
   otherwise renders the Test View.
------------------------------------------------------------------ */
const APlusCore2TestPage = () => {
  const { testId } = useParams();
  return testId ? <TestView testId={testId} /> : <APlusCore2TestList />;
};
...
const TestView = ({ testId }) => {
  ...
  useEffect(() => {
    ...
    const progress = {
      ...
      score,
      totalQuestions,
      category: currentTest?.category || "aplus2",
  ...
  /* ------------------------------------------------------------------
     (7) Filtered Questions for Review
  ------------------------------------------------------------------ */
      ...
      <div className="score-overlay">
        <div className="score-content">
          ...
          <div className="overlay-buttons">
            ...
            {/* Link back to /practice-tests/aplus-core2 */}
            <button
              className="back-btn"
              onClick={() => navigate("/practice-tests/aplus-core2")}
            >
              Back to Test List
            </button>
            {/* Let testId go up as you see fit */}
            {Number(testId) < 9999 && (
              <button
                className="next-test-button"
                onClick={() =>
                  navigate(`/practice-tests/aplus-core2/${Number(testId) + 1}`)
                }
              >
                Next Test
              </button>
  ...
  return error ? (
    ...
    <div className="aplus-test-container">
      ...
      <div className="upper-control-bar">
        ...
        {/* Link back to /practice-tests/aplus-core2 */}
        <button className="back-btn" onClick={() => navigate("/practice-tests/aplus-core2")}>
          Back to Test List
        </button>
      </div>
