// src/components/GlobalTestList.js
...
const GlobalTestList = ({ 
  ...
  // NEW FUNCTIONALITY: Check for just-completed tests and force refresh
  useEffect(() => {
    if (location.state?.testJustCompleted) {
      fetchAttempts();
      // Clean up the state to prevent repeated refreshes
    ...
    <div className="testlist-container">
      <div className="testlist-header">
        <div className="testlist-title-section">
          <h1 className="testlist-title">{title}</h1>
          <p className="testlist-subtitle">{subtitle}</p>
        </div>
