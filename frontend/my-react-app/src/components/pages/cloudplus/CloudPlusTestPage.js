// src/components/pages/cloudplus/CloudPlusTestPage.js
...
// Import the test list component for Cloud+
...
/* ------------------------------------------------------------------
   Main Component: CloudPlusTestPage
   If no :testId => show test list, else show TestView
------------------------------------------------------------------ */
const CloudPlusTestPage = () => {
  const { testId } = useParams();
  return testId ? <TestView testId={testId} /> : <CloudPlusTestList />;
};
...
const TestView = ({ testId }) => {
      ...
      try {
        ...
        if (savedProgressStr) {
          ...
          const initProgress = {
            ...
            category: data.category || "cloudplus",
  ...
  useEffect(() => {
    ...
    const progress = {
      ...
      score,
      totalQuestions,
      category: currentTest?.category || "cloudplus",
      ...
      <div className="score-overlay">
        <div className="score-content">
          ...
          <div className="overlay-buttons">
            ...
            {/* Link back to /practice-tests/cloud-plus */}
            <button
              className="back-btn"
              onClick={() => navigate("/practice-tests/cloud-plus")}
            >
              Back to Test List
            </button>
            {Number(testId) < 9999 && (
              <button
                ...
                onClick={() =>
                  navigate(`/practice-tests/cloud-plus/${Number(testId) + 1}`)
                }
              >
                Next Test
              </button>
  ...
  return error ? (
    ...
    <div className="cloudplus-test-container">
      ...
      <h1 className="cloudplus-title">{currentTest.testName}</h1>
