// src/components/pages/casp/CaspPlusTestPage.js
import React, {
  useState,
  useEffect,
  useCallback,
...
/* ------------------------------------------------------------------
   Main Component: CaspPlusTestPage
   If no :testId => show test list,
   else show the test view
------------------------------------------------------------------ */
const CaspPlusTestPage = () => {
  const { testId } = useParams();
  return testId ? <TestView testId={testId} /> : <CaspPlusTestList />;
};
...
const TestView = ({ testId }) => {
      ...
      try {
        ...
        if (savedProgressStr) {
          ...
          const initProgress = {
            ...
            category: data.category || "caspplus",
  ...
  useEffect(() => {
    ...
    const progress = {
      ...
      score,
      totalQuestions,
      category: currentTest?.category || "caspplus",
      ...
      <div className="score-overlay">
        <div className="score-content">
          ...
          <div className="overlay-buttons">
            ...
            {/* Link back to /practice-tests/casp-plus */}
            <button
              className="back-btn"
              onClick={() => navigate("/practice-tests/casp-plus")}
            >
              Back to Test List
            </button>
            {Number(testId) < 9999 && (
              <button
                ...
                onClick={() =>
                  navigate(`/practice-tests/casp-plus/${Number(testId) + 1}`)
                }
              >
                Next Test
              </button>
  ...
  return error ? (
    ...
    <div className="caspplus-test-container">
      ...
      <h1 className="caspplus-title">{currentTest.testName}</h1>
