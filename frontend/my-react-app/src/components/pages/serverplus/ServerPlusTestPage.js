// src/components/pages/serverplus/ServerPlusTestPage.js
...
// Import the matching test list component for Server+
...
/* ------------------------------------------------------------------
   Main Component: ServerPlusTestPage
   If no :testId => show test list, else show the test view
------------------------------------------------------------------ */
const ServerPlusTestPage = () => {
  const { testId } = useParams();
  return testId ? <TestView testId={testId} /> : <ServerPlusTestList />;
};
...
const TestView = ({ testId }) => {
      ...
      try {
        ...
        if (savedProgressStr) {
          ...
          const initProgress = {
            ...
            category: data.category || "serverplus",
  ...
  /* ------------------------------------------------------------------
     (6) Debounced Save Partial Progress
  ------------------------------------------------------------------ */
  useEffect(() => {
    ...
    const progress = {
      ...
      score,
      totalQuestions,
      category: currentTest?.category || "serverplus",
  ...
  const finishTestProcess = useCallback(() => {
    ...
    // Mark test as finished in the DB
      ...
      <div className="score-overlay">
        <div className="score-content">
          ...
          <div className="overlay-buttons">
            ...
            {/* Navigate back to /practice-tests/server-plus */}
            <button
              className="back-btn"
              onClick={() => navigate("/practice-tests/server-plus")}
            >
              Back to Test List
            </button>
            {Number(testId) < 9999 && (
              <button
                ...
                onClick={() =>
                  navigate(`/practice-tests/server-plus/${Number(testId) + 1}`)
                }
              >
                Next Test
              </button>
  ...
  return error ? (
    ...
    <div className="serverplus-test-container">
      ...
      <h1 className="serverplus-title">{currentTest.testName}</h1>
