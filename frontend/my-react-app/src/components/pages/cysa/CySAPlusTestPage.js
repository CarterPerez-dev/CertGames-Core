// src/components/pages/cysa/CySAPlusTestPage.js
import React, {
  useState,
  useEffect,
  useCallback,
...
/* ------------------------------------------------------------------
   Main Component: CySAPlusTestPage
   If no :testId => show list, else show TestView
------------------------------------------------------------------ */
const CySAPlusTestPage = () => {
  const { testId } = useParams();
  return testId ? <TestView testId={testId} /> : <CySAPlusTestList />;
};
...
const TestView = ({ testId }) => {
  ...
  useEffect(() => {
    const fetchTestData = async () => {
      ...
      try {
        ...
        // Assign fallback question IDs if needed
  ...
  useEffect(() => {
    ...
    const progress = {
      ...
      score,
      totalQuestions,
      category: currentTest?.category || "cysa",
      ...
      <div className="score-overlay">
        <div className="score-content">
          ...
          <div className="overlay-buttons">
            ...
            {/* Link back to /practice-tests/cysa-plus */}
            <button
              className="back-btn"
              onClick={() => navigate("/practice-tests/cysa-plus")}
            >
              Back to Test List
            </button>
            {Number(testId) < 9999 && (
              <button
                ...
                onClick={() =>
                  navigate(`/practice-tests/cysa-plus/${Number(testId) + 1}`)
                }
              >
                Next Test
              </button>
  ...
  return error ? (
    ...
    <div className="cysa-test-container">
      ...
      <h1 className="cysa-title">{currentTest.testName}</h1>
