// src/components/pages/awscloud/AWSCloudTestPage.js
...
// Import the matching test list component for AWS Cloud
...
/* ------------------------------------------------------------------
   Main Component: AWSCloudTestPage
   If no :testId => show test list, else show the Test View
------------------------------------------------------------------ */
const AWSCloudTestPage = () => {
  const { testId } = useParams();
  return testId ? <TestView testId={testId} /> : <AWSCloudTestList />;
};
...
const TestView = ({ testId }) => {
  ...
  // Attempt to fetch shop items if not already
  useEffect(() => {
    if (shopStatus === "idle") {
      ...
      try {
        ...
        if (savedProgressStr) {
          ...
          const initProgress = {
            ...
            category: data.category || "awscloud",
  ...
  useEffect(() => {
    ...
    const progress = {
      ...
      score,
      totalQuestions,
      category: currentTest?.category || "awscloud",
      ...
      <div className="score-overlay">
        <div className="score-content">
          ...
          <div className="overlay-buttons">
            ...
            {/* Link back to /practice-tests/awscloud */}
            <button
              className="back-btn"
              onClick={() => navigate("/practice-tests/awscloud")}
            >
              Back to Test List
            </button>
            {Number(testId) < 9999 && (
              <button
                ...
                onClick={() =>
                  navigate(`/practice-tests/awscloud/${Number(testId) + 1}`)
                }
              >
                Next Test
              </button>
  ...
  return error ? (
    ...
    <div className="aplus-test-container">
      ...
      <h1 className="awscloud-title">{currentTest.testName}</h1>
