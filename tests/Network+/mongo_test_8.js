db.tests.insertOne({
  ...
  "questions": [
    ...
    },
    {
      ...
      },
      {
       "id": 7,
        ...
        "explanation": "To support at least 2000 hosts, you need enough host bits. Here's the breakdown: /24 (255.255.255.0): 8 host bits = 2^8 - 2 = 254 usable hosts (too small) /23 (255.255.254.0): 9 host bits = 2^9 - 2 = 510 usable hosts (too small) /22 (255.255.252.0): 10 host bits = 2^10 - 2 = 1022 usable hosts (too small) /21 (255.255.248.0): 11 host bits = 2^11 - 2 = 2046 usable hosts (This meets the requirement) /16 is the original network, providing far more than needed, so its not the *most* appropriate.",
      ...
      },
    {
      "id": 43,
       "question": "You are troubleshooting a network where users report that they can access some websites but not others.  You suspect a DNS problem.  Which command-line tool would you use to query a *specific* DNS server and resolve a *specific* domain name, allowing you to test different DNS servers and diagnose resolution issues?",
        ...
        "examTip": "Use `nslookup` or `dig` with specific domain names and DNS server addresses to diagnose DNS resolution problems."
      },
     {
      "id": 44,
       ...
       "question":"What is 'port mirroring' (also known as 'SPAN') on a network switch used for?",
        "options":[
          "To encrypt network traffic.",
           ...
           "To copy network traffic from one or more source ports to a designated destination port, allowing for monitoring and analysis of the traffic without disrupting normal network operation. This is often used with intrusion detection systems (IDSs), intrusion prevention systems (IPSs), or protocol analyzers.",
           "To assign IP addresses dynamically."
        ],
        "correctAnswerIndex": 2,
        "explanation":"Port mirroring is a powerful troubleshooting and monitoring technique. It allows you to *duplicate* network traffic from one or more switch ports (the *source* ports) to another port (the *destination* port). You then connect a network analyzer (like Wireshark), an IDS/IPS, or another monitoring device to the destination port to capture and analyze the traffic *without* affecting the normal flow of data on the source ports. It's *not* encryption, port security, or DHCP.",
      ...
      },
    {
      ...
      },
     {
       "id": 69,
        ...
        "A technique for speeding up DHCP address assignment.",
    ...
    },
    {
     ...
     {
       "id": 76,
        "question": "You are designing a wireless network for a large office building.  You need to ensure good coverage and minimize interference.  You are using the 2.4 GHz band.  How many *non-overlapping* channels are available, and which channels are they?",
        "options":[
    ...
    },
    {
    ...
    },
     {
       "id": 78,
        ...
        "correctAnswerIndex": 2, // Most accurate and specific to the scenario
      ...
      },
      {
        ...
        "correctAnswerIndex": 2,
        "explanation": "The `show spanning-tree` command on a Cisco switch provides detailed information about the Spanning Tree Protocol (STP) operation. This includes: *The root bridge:* The switch elected as the central reference point for STP. *Bridge ID:* The unique identifier of the switch. *Port roles:* Root port (closest to the root bridge), designated port (forwarding traffic for a segment), blocking port (blocking traffic to prevent loops). *Port states:* Forwarding, blocking, learning, listening, disabled. *Other STP parameters:* Timers, costs, etc. This information is essential for diagnosing STP-related problems like loops or slow convergence. `show ip interface brief` shows interface status and IP addresses, `show vlan brief` shows VLAN assignments, and `show mac address-table` shows learned MAC addresses.",
    ...
    },
    {
       ...
       "question": "A user reports that they can access some websites but not others. You suspect a DNS issue. Which command-line tool, and what specific syntax, would you use to query a *specific* DNS server (e.g., Google's public DNS server at 8.8.8.8) to resolve a *specific* domain name (e.g., `www.example.com`)?",
        ...
        "correctAnswerIndex": 2,
        "explanation": "`nslookup` (or `dig` on Linux/macOS) is specifically designed to query DNS servers. The syntax `nslookup [domain name] [DNS server]` allows you to specify *both* the domain you want to resolve *and* the DNS server you want to query. This is crucial for troubleshooting DNS issues, as it allows you to test different DNS servers and isolate the problem. `ping` tests basic connectivity, `tracert` shows the route, and `ipconfig /all` shows your *current* DNS settings, but doesn't actively *test* resolution against a specific server.",
      ...
      },
    {
    ...
    },
     {
      "id": 92,
       ...
       "correctAnswerIndex": 1,
       "explanation": "A honeypot is a *deception* technique used in cybersecurity. It's a deliberately vulnerable system or network resource that *mimics* a legitimate target (like a server or database). It's designed to *lure* attackers, allowing security researchers to: *Observe their techniques:* See how attackers try to compromise systems. *Gather threat intelligence:* Learn about new exploits and attack methods. *Divert attackers:* Potentially distract them from real, valuable targets. It's *not* a secure server, a firewall that blocks *all* traffic, or an encryption tool.",
    ...
    "correctAnswerIndex": 1,
    "explanation": "Here's the breakdown of why the correct answer is the most secure and correct: 1. **`line vty 0 4`**:  This enters the configuration mode for the virtual terminal lines (VTY lines 0 through 4), which are used for remote access (like SSH and Telnet). 2. **`transport input ssh`**: This is a *crucial security best practice*. It restricts remote access to *only* SSH, disabling the insecure Telnet protocol. 3. **`access-list 10 permit 192.168.1.0 0.0.0.255`**:  This creates an access control list (ACL) named '10'.  This specific line *permits* traffic originating from the 192.168.1.0/24 network.  The `0.0.0.255` is a *wildcard mask*, which is the inverse of a subnet mask. It means 'match any host within the 192.168.1.0/24 network'. 4. **`access-class 10 in`**: This applies the ACL named '10' to the *incoming* traffic on the VTY lines. This means that only traffic matching the ACL (i.e., originating from 192.168.1.0/24) will be allowed to establish an SSH connection to the router. Option A allows *all* protocols on the VTY lines, which is insecure. Option C uses `permit any`, which is extremely insecure as it allows SSH access from *any* IP address. Option D is applying the ACL to the *console* line (physical console port), not the VTY lines for remote access. Also, the placement of the access-list before applying with `access-class` is important for clarity and standard practice",
    "examTip": "To restrict SSH access on a Cisco router, use `transport input ssh` and an ACL applied to the VTY lines with `access-class`."
    },
     {
        ...
        "correctAnswerIndex": 2,
        "explanation": "Since the servers are on the *same subnet*, routing is *not* involved. This eliminates DNS. DHCP is for IP *assignment*, not ongoing performance. The most likely causes of slow transfers *within* the same subnet are: 1. *Speed/Duplex Mismatch:* If the server NIC and switch port are configured for different speeds or duplex settings (e.g., one is full-duplex, the other is half-duplex), this will cause collisions and *significant* performance degradation. Auto-negotiation *should* prevent this, but it can sometimes fail. 2. *Interface Errors:* Increasing error counters (CRC errors, runts, giants, etc.) on either the server NIC or the switch port indicate a *physical layer* problem (bad cable, faulty NIC, bad port). 3. *Cable Issues:* A faulty cable can cause packet loss and retransmissions. A cable tester can verify cable integrity. Rebooting the switch is a last resort, and while it *might* temporarily fix some issues, it doesn't *diagnose* the root cause.",
    ...
    },
     {
         ...
         "correctAnswerIndex": 2, // Correct, complete and best practice
         "explanation": "The correct configuration needs to define a DHCP pool, specify the network and subnet mask, set the default gateway, and configure the DNS servers. It's also best practice to exclude a range of addresses for static assignments: Option A is missing the default gateway and DNS server configurations. Option B is complete but lacks address exclusion, which is a best practice. Option D is complete for the requested info, but still a best practice to exclude addresses, *and* having two separate `dns-server` lines is less common than listing both servers on one line. Option C includes all the necessary components AND a best practice: `ip dhcp pool MYPOOL`: Creates a DHCP pool named MYPOOL. `network 192.168.1.0 255.255.255.0`: Defines the network and subnet mask for the DHCP pool. `default-router 192.168.1.1`: Sets the default gateway for DHCP clients. `dns-server 8.8.8.8 8.8.4.4`: Sets the DNS servers for DHCP clients. `ip dhcp excluded-address 192.168.1.1 192.168.1.10`: Excludes the IP addresses from 192.168.1.1 to 192.168.1.10 from being assigned by DHCP. This is good practice to reserve addresses for statically assigned devices (like the router itself, servers, printers).",
    ...
    },
    {
        ...
        "correctAnswerIndex": 3,
        "explanation": "If the VPN tunnel were *not established at all*, there would be *no* connectivity between the sites. Successful pings indicate that *some* level of connectivity exists through the tunnel. The *most likely* causes of *partial* connectivity are: *Firewall rules:* One or both VPN gateways might have firewall rules that block *specific* types of traffic (e.g., certain ports or protocols). *Routing problems:* The routing tables on the VPN gateways might not be configured correctly, so some subnets on one side might not be reachable from the other side. *MTU mismatch:* This can cause packet loss and intermittent connectivity, particularly for larger packets. Option D is the *least* likely because pings are successful, proving the tunnel exists.",
