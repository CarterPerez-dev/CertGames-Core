...
*******Ok so we have 10 tests with 100 questiosn each, they range in diffuclty and test 1 isnt on tyeh ficculty sca;e- its suypposed to exactly on par witht eh actual real life exam. so its labeled "normal", then test 2 starts at "very easy" and then increases in diffculty until teh hardest ets which is test 10 labeled "ultra level". so what i need you to do is give me test 6 rigth now which is consiered "Intermediate" but still somehwat relative to the CISSP exam difficulty******** however im just gonna give you test 4 and 5 which is "moderate" and test 4 which is "intermediate" adn then gauge those questiosn diffuclty and make tets 6 slighly harder than test 5


so here is test 4 and 5 so you know not to duplciate any questions from test 4&5 and also know the difficulty of questions you shoudl make etst 6
