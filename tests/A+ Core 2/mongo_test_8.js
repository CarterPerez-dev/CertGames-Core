...
db.tests.insertOne({
  ...
  "questions": [
    ...
    },
    {
      ...
      "options": [
        "`/var/log/messages`",
        "`/var/log/auth.log` (or `/var/log/secure` on some systems) for login/authentication events, and potentially `/var/log/audit/audit.log` (if auditd is configured) or user's `~/.bash_history` files (with caution)",
        "`/var/log/syslog`",
        "`/var/log/dmesg`"
      ],
      "correctAnswerIndex": 1,
