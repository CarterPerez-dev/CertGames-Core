db.tests.insertOne({
  ...
  "questions": [
    ...
    },
    {
      ...
      "correctAnswerIndex": 2,
      "explanation": "Amazon Redshift would be most appropriate for this data warehouse requirement. Redshift is a fully managed, petabyte-scale data warehouse service designed specifically for analytics and handling complex queries on large datasets. It uses columnar storage, data compression, and massively parallel processing (MPP) architecture to deliver fast query performance on datasets ranging from gigabytes to petabytes. Redshift is optimized for high-performance analysis of structured data through SQL queries, making it ideal for a 2 PB data warehouse with complex analytical query requirements. Amazon RDS for PostgreSQL (option 0) is a relational database service suitable for transactional workloads but not optimized for petabyte-scale analytics or data warehousing needs. Amazon Athena (option 1) is a serverless query service that allows analysis of data in S3 using standard SQL, but while it's good for ad-hoc queries, it may not deliver the same level of performance as Redshift for complex analytics on 2 PB of data, especially for recurring query patterns. Amazon DynamoDB (option 3) is a NoSQL database service designed for high-throughput, low-latency applications requiring key-value or document data models, not for complex analytical queries across petabytes of data.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 2,
      "explanation": "EC2 instances across multiple Availability Zones with an Application Load Balancer and Auto Scaling should be implemented for high availability that can withstand Availability Zone failures. This architecture distributes EC2 instances across multiple Availability Zones within a region, ensuring that if one AZ fails, instances in other AZs continue to serve traffic. The Application Load Balancer automatically detects unhealthy instances and routes traffic only to healthy instances, redirecting traffic away from a failed AZ to functioning AZs. Auto Scaling maintains the desired capacity by launching replacement instances in functioning AZs if an AZ failure reduces the number of healthy instances. Together, these components create a resilient architecture that can continue operating through an AZ failure without disruption. A single EC2 instance with EBS snapshots and Route 53 health checks (option 0) would experience downtime during an AZ failure as the instance would be unavailable, and recovery would require launching a new instance from snapshots. Multiple EC2 instances in a single Availability Zone with Auto Scaling (option 1) would not protect against an AZ failure, as all instances would be affected by the failure of that single AZ. Amazon S3 for static content with CloudFront distribution (option 3) provides high availability for static content but doesn't address the compute needs of an application that requires EC2 instances.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 3,
      "explanation": "Multi-Site Active/Active provides the fastest recovery time with minimal data loss but requires running systems in multiple regions. In a Multi-Site Active/Active disaster recovery approach, the application is deployed and actively serving traffic from multiple AWS regions simultaneously. Data is replicated across regions to maintain consistency, and DNS routing (typically through Amazon Route 53) distributes traffic between the regions. This approach provides near-zero recovery time objective (RTO) and recovery point objective (RPO) since all sites are already up and running with current data. When a region experiences an outage, traffic is simply routed to the healthy region(s) with minimal or no disruption to users. The trade-off is that this approach is the most expensive, as it requires maintaining full production capacity in multiple regions. Backup and Restore (option 0) involves regular backups that are restored in case of disaster, typically resulting in hours or days of recovery time and potential data loss since the last backup. Pilot Light (option 1) keeps core systems running in the recovery region but requires additional components to be deployed during recovery, typically resulting in recovery times of tens of minutes to hours. Warm Standby (option 2) maintains a scaled-down but functional copy of the environment in the recovery region, providing faster recovery than Pilot Light but still requiring some time to scale up to full capacity.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 2,
      "explanation": "Signing a Business Associate Addendum (BAA) with AWS is the prerequisite step the company must complete before using AWS for PHI. Under HIPAA (Health Insurance Portability and Accountability Act), a Business Associate is a person or entity that performs certain functions or activities that involve the use or disclosure of protected health information. AWS can act as a Business Associate for HIPAA-covered entities, but this relationship must be formalized through a BAA. AWS requires customers to execute an AWS BAA before storing, processing, or transmitting PHI on AWS. Without a signed BAA, customers should not store any PHI on AWS, regardless of what security measures they implement. Implementing encryption at rest (option 0) is an important security measure for PHI but is not the prerequisite legal step required before using AWS for PHI. Configuring VPC Flow Logs and CloudTrail logging (option 1) provides important audit trails but is not the prerequisite legal requirement. Deploying all resources in AWS GovCloud (US) region (option 3) is not required for HIPAA compliance; while GovCloud supports regulated workloads, PHI can be stored in other AWS regions as long as a BAA is in place and HIPAA-eligible services are used.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 1,
      "explanation": "AWS Key Management Service (KMS) provides a centralized way to create and manage encryption keys used across various AWS services. KMS makes it easy to create and manage cryptographic keys and control their use across a wide range of AWS services and in your applications. It is integrated with numerous AWS services including S3, EBS, RDS, DynamoDB, Lambda, and many others, allowing you to use the same keys consistently across different services. KMS enables you to create, rotate, disable, and define usage policies for the keys that protect your data, providing centralized control over your encryption strategy. AWS Certificate Manager (option 0) provides SSL/TLS certificates for securing network communications but doesn't manage encryption keys for data at rest. AWS Secrets Manager (option 2) helps you protect secrets like database credentials and API keys but isn't designed as a general-purpose encryption key management service. Server-Side Encryption with Amazon S3-Managed Keys (SSE-S3) (option 3) is an encryption option specific to Amazon S3 that uses keys managed entirely by S3, not a centralized key management service for multiple AWS services.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 3,
      "explanation": "AWS Systems Manager Parameter Store should be used to securely manage and retrieve sensitive parameters for CloudFormation templates. Parameter Store provides secure, hierarchical storage for configuration data management, including sensitive data like passwords, database strings, and license codes. It integrates directly with CloudFormation, allowing you to reference Parameter Store parameters in your templates using the dynamic reference pattern, avoiding hardcoded secrets. Parameter Store supports different tiers of parameters with varying sizes and features, encryption using AWS KMS, parameter versioning, and fine-grained access control through IAM policies. This makes it well-suited for managing configuration data for CloudFormation deployments. AWS Secrets Manager (option 0) provides similar functionality for storing secrets with the additional capability of automatic rotation, but CloudFormation has more established integration patterns with Parameter Store, particularly for configuration data beyond just credentials. AWS CloudTrail (option 1) records API activity for auditing purposes but doesn't provide storage for configuration data. Amazon S3 with server-side encryption (option 2) could store configuration files securely, but it lacks the parameter management features and direct CloudFormation integration that Parameter Store provides.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 3,
      "explanation": "EBS snapshots can be shared across AWS accounts, while RDS automated backups cannot be shared most accurately describes a key difference between these backup types. Amazon EBS snapshots can be shared with other AWS accounts or made public, allowing you to share a point-in-time backup of a volume with others or copy it to another region. This provides flexibility for collaboration, resource sharing, or cross-region disaster recovery. In contrast, RDS automated backups cannot be directly shared with other AWS accounts. If you need to share an RDS database backup with another account, you need to create a DB snapshot (which is different from automated backups) and then share that snapshot. EBS snapshots are always stored in Amazon S3, while RDS backups are stored locally on the instance (option 0) is incorrect; both EBS snapshots and RDS automated backups are stored in Amazon S3, though the storage is managed by AWS and not directly accessible through your S3 buckets. EBS snapshots are full backups each time, while RDS automated backups are incremental after the first full backup (option 1) is incorrect; both EBS snapshots and RDS automated backups use incremental approaches where only the blocks that have changed since the last backup are stored. EBS snapshots can only be created manually, while RDS automated backups can be scheduled (option 2) is incorrect; EBS snapshots can be automated using Amazon Data Lifecycle Manager or custom scheduling scripts, not just created manually.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 0,
      "explanation": "Amazon RDS with Multi-AZ deployment would provide the required high availability to withstand an Availability Zone failure. In a Multi-AZ deployment, RDS automatically provisions and maintains a synchronous standby replica in a different Availability Zone. The primary instance replicates synchronously to the standby replica, ensuring that the standby is up-to-date with the primary. In the event of a planned maintenance, DB instance failure, or Availability Zone failure, RDS automatically fails over to the standby instance, typically completing the failover within 1-2 minutes. This design ensures the application's database remains operational even when an entire Availability Zone experiences a failure, meeting the high-availability requirement. Amazon RDS with Read Replicas (option 1) enhances read scalability and can serve as part of a disaster recovery solution, but replicas are asynchronous and don't automatically fail over, making them unsuitable as the sole high-availability solution. Amazon DynamoDB with on-demand capacity (option 2) provides high availability across multiple Availability Zones by default, but the question specifically refers to enhancing availability for an existing application, suggesting a relational database context. Amazon RDS with automated backups (option 3) enables point-in-time recovery but doesn't provide automatic failover during an Availability Zone failure, resulting in downtime while a new instance is created from backups.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 2,
      "explanation": "AWS CodePipeline provides a continuous integration and continuous delivery (CI/CD) pipeline for automating the build, test, and deploy process. CodePipeline is a fully managed continuous delivery service that helps you automate your release pipelines for fast and reliable application and infrastructure updates. It automatically triggers your pipeline when a change is detected in your source code repository, builds the application, runs automated tests, and deploys to your specified environments. CodePipeline can be integrated with AWS services like CodeBuild for building and testing code, and CodeDeploy for automating deployments, as well as with third-party tools like GitHub, Jenkins, and others. This comprehensive pipeline automation directly addresses the requirement for automating the build, test, and deploy process. AWS CloudFormation (option 0) automates the provisioning and management of infrastructure resources but doesn't provide the end-to-end CI/CD pipeline capabilities needed for application development processes. AWS OpsWorks (option 1) is a configuration management service using Chef or Puppet for automating how servers are configured, deployed, and managed, but it's not a CI/CD pipeline service. AWS Elastic Beanstalk (option 3) simplifies the deployment and management of applications but doesn't provide the comprehensive build, test, and deploy pipeline that CodePipeline offers.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 2,
      "explanation": "AWS Budgets should be used for creating custom budgets and receiving alerts when costs exceed defined thresholds. Budgets enables you to set custom budgets to track your costs and usage by time period, cost allocation tags, accounts, services, and more. It allows you to define alert thresholds and receive notifications via email or through Amazon SNS when actual or forecasted costs exceed these thresholds. You can create various types of budgets, including cost budgets, usage budgets, reservation budgets, and Savings Plans budgets, providing comprehensive coverage for different cost control scenarios. This directly addresses the requirement for custom budgets and threshold-based alerts. AWS Cost Explorer (option 0) provides visualization and analysis of costs and usage data but doesn't offer the budget creation and threshold alerting capabilities that Budgets provides. AWS Trusted Advisor (option 1) offers recommendations across multiple categories including cost optimization but doesn't provide budget creation and threshold alerting. AWS Cost and Usage Report (option 3) provides detailed cost and usage data for analysis but doesn't include budgeting and alerting functionality.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 2,
      "explanation": "AWS Service Quotas is where they should look to find detailed information about service quotas and limits. Service Quotas is a service that helps you manage your quotas for over 100 AWS services from one location. It provides a central location to view and manage your quotas (also referred to as limits in some AWS services) across AWS services, displaying the quota values, showing which quotas you've used, and allowing you to request quota increases for adjustable quotas. Service Quotas also provides an API for programmatically managing quotas, enabling integration with your own systems. This makes it the most comprehensive and authoritative source for service quota information. AWS Health Dashboard (option 0) provides information about AWS service health and planned maintenance activities but doesn't focus on service quotas and limits. AWS Trusted Advisor (option 1) includes a service limits check that identifies resources that have approached or exceeded their service quotas, but it doesn't provide comprehensive documentation on all service quotas and their values. AWS Support Center (option 3) provides access to support resources and can be used to request quota increases, but it's not the primary location for documentation on service quotas.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 3,
      "explanation": "AWS Artifact should be used to access security and compliance reports and regulatory information about AWS services. Artifact is the go-to resource for compliance-related information, providing on-demand access to AWS security and compliance documents such as AWS ISO certifications, Payment Card Industry (PCI) reports, and Service Organization Control (SOC) reports. It also allows customers to review, accept, and manage agreements with AWS, such as the Business Associate Addendum (BAA) for HIPAA compliance. While technical documentation is available through AWS Documentation, the specific requirements for security, compliance, and regulatory information point to AWS Artifact as the appropriate resource. AWS Support Center (option 0) provides access to support resources and documentation but isn't specifically focused on compliance documentation. AWS Documentation (option 1) provides technical information about AWS services including user guides, developer guides, and API references, but doesn't include the compliance reports and regulatory information available through Artifact. AWS Management Console (option 2) is the web interface for accessing and managing AWS services but isn't a documentation resource itself.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 2,
      "explanation": "Business Support meets the requirements for 24/7 access to technical support via phone, email, and chat at the lowest cost. AWS Business Support provides full access to AWS Trusted Advisor checks, access to guidance, configuration, and troubleshooting of AWS interoperability with third-party software, and 24/7 access to Cloud Support Engineers via phone, email, and chat with a 1-hour response time for urgent issues. This directly addresses the requirement for round-the-clock technical support for production workloads across all communication channels. Developer Support (option 0) provides access to technical support via email during business hours only, with a 12-24 hour response time for normal cases, which doesn't meet the 24/7 support requirement. Basic Support (option 1) includes access to customer service, documentation, whitepapers, and support forums, but doesn't provide technical support from AWS representatives. Enterprise Support (option 3) provides the highest level of support with the fastest response times and additional features like a Technical Account Manager, but at a higher cost than Business Support, making it not the lowest cost option that meets the requirements.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 3,
      "explanation": "S3 Access Points should be implemented for fine-grained access control based on user attributes. S3 Access Points are named network endpoints attached to S3 buckets that simplify managing data access at scale for applications with shared datasets on S3. Each access point has its own IAM policy that can grant different permissions to different users or groups for the same underlying bucket. Access Points can be configured with specific permissions based on user attributes, allowing distinct access control for different users accessing the same bucket. This enables granular permissions management without needing to manage a complex bucket policy. S3 Bucket Policies (option 0) can provide fine-grained access control but become complex and difficult to manage for scenarios with many different permission requirements for different users accessing the same data. S3 ACLs (option 1) are a legacy access control mechanism with limited capabilities compared to bucket policies and access points, and AWS recommends using bucket policies and access points instead. S3 Lifecycle Policies (option 2) manage the transition of objects between storage classes and their expiration, but don't provide access control functionality.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 0,
      "explanation": "Amazon Kinesis Data Streams is most suitable for ingesting and processing real-time streaming data from IoT devices and mobile applications. Kinesis Data Streams is a massively scalable and durable real-time data streaming service specifically designed to continuously capture gigabytes of data per second from thousands of sources. It can handle high-throughput, real-time data such as video, audio, application logs, website clickstreams, and IoT telemetry data. Kinesis Data Streams stores the data for later processing and enables real-time analytics through integration with services like Kinesis Data Analytics or custom processing applications. It maintains the order of records within shards, which is often important for time-series data from IoT devices. Amazon SQS (option 1) is a message queuing service that's useful for decoupling and scaling microservices, but it doesn't maintain record order (unless using FIFO queues with throughput limitations) and isn't optimized for high-volume streaming data ingestion and real-time processing. Amazon SNS (option 2) is a pub/sub messaging service for sending notifications to subscribing endpoints or clients, not for continuous ingestion and processing of streaming data. AWS Batch (option 3) is designed for batch processing jobs that can run asynchronously, not for real-time streaming data processing.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 2,
      "explanation": "Business Support meets these requirements at the lowest cost. AWS Business Support provides access to architectural guidance and application architecture recommendations through several features: access to AWS Trusted Advisor, which offers best practice recommendations across performance, security, cost optimization, and fault tolerance; access to the Infrastructure Event Management service for additional guidance for specific events; and access to AWS Support Engineers with deep technical knowledge of AWS services and how they work together. Business Support customers can open technical support cases and receive guidance on how services can be used for specific use cases, implementation help, and architecture reviews. Developer Support (option 0) provides technical support for development issues, but doesn't include full access to Trusted Advisor or architectural guidance for production systems. Basic Support (option 1) provides access to customer service, documentation, whitepapers, and support forums, but doesn't include access to technical support representatives for architectural guidance. Enterprise Support (option 3) provides the highest level of support with additional features like a Technical Account Manager and shorter response times, but at a significantly higher cost than Business Support, making it not the lowest cost option that meets the requirements.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 1,
      "explanation": "Amazon CloudFront is a Content Delivery Network (CDN) that caches content at edge locations around the world, reducing latency for viewers regardless of their geographic location. It also integrates seamlessly with Amazon S3, making it the ideal choice for delivering on-demand video with minimal delay. S3 Transfer Acceleration (option 0) speeds up transfers to and from S3 by routing traffic through Amazon’s network, but it is not a CDN solution. AWS Global Accelerator (option 2) is used to improve availability and performance of non-HTTP/HTTPS applications by routing traffic through edge locations, but it is not optimized for caching static content. AWS Direct Connect (option 3) is a dedicated network connection to AWS, not a global caching solution.",
    ...
    },
    {
      ...
      "correctAnswerIndex": 1,
      "explanation": "An AWS Site-to-Site VPN connection provides a secure (encrypted) tunnel between on-premises infrastructure and an Amazon VPC, running over existing internet connections. This approach meets the requirement for encryption in transit without the need for dedicated network circuits. AWS Direct Connect (option 0) is a private connection that does not inherently encrypt data and requires a physical link. AWS Client VPN (option 2) is designed primarily for end users to securely access resources, not for data center-level connectivity. VPC peering (option 3) is used to connect two VPCs, not to connect on-premises networks to a VPC.",
